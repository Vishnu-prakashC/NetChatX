...
const messageCtrl = {
  ...
  // Create or fetch a direct message conversation (upsert)
  createConversation: async (req, res) => {
    try {
      const { recipient } = req.body;
      if (!recipient) return res.status(400).json({ msg: 'Recipient is required' });
      ...
      const conversation = await Conversations.findOneAndUpdate(
        ...
        {
          $setOnInsert: { recipients: [userId, recipient], createdAt: new Date() },
