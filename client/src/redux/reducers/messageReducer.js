...
const messageReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case MESSAGE_TYPES.ADD_MESSAGE:
      ...
      // Prevent duplicates: if message with same _id already exists, ignore
      if (action.payload._id && state.data.some(m => m._id === action.payload._id)) {
        return state;
      }

      // If message has a tempId and a matching optimistic message exists, replace it
      if (action.payload.tempId && state.data.some(m => m.tempId === action.payload.tempId)) {
        return {
          ...state,
          data: state.data.map(m => m.tempId === action.payload.tempId ? { ...action.payload } : m),
          users: state.users.map((user) =>
            user._id === (action.payload.recipient || action.payload.sender) ||
            user._id === (action.payload.sender || action.payload.recipient)
