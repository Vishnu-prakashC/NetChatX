...
const MessageSearch = ({ onUserSelect, onClose }) => {
  ...
  const handleUserSelect = (user) => {
    (async () => {
      // Prevent double clicks from issuing duplicate createConversation calls
      if (handleUserSelect.creating) return;
      handleUserSelect.creating = true;

      addToRecentSearches(user.username);

      // Ensure a conversation exists for this user before notifying parent
      try {
        const res = await postDataAPI('conversations', { recipient: user._id }, auth.token);
        // Only log the conversation id to reduce noisy output
        console.debug('MessageSearch: created conversation id=', res?.data?.conversation?._id);

        // Attach conversation info to user object so parent can use it if desired
        if (res?.data?.conversation) {
          user.conversation = res.data.conversation;
        }
      } catch (err) {
        console.error('MessageSearch: createConversation failed', err?.response?.data || err.message);
        // proceed anyway — parent should handle missing conversation gracefully
      } finally {
        handleUserSelect.creating = false;
      }
