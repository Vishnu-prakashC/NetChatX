...
const SocketServer = (socket) => {
  ...
  // Handle incoming direct/group messages from clients
  // Clients emit 'addMessage' (see client Redux action `addMessage`) with an acknowledgement callback
  socket.on("addMessage", async (msg, callback) => {
    console.log(`💬 Unified message:`, { conversationId: msg.conversationId, sender: msg.sender, recipient: msg.recipient, tempId: msg.tempId, text: msg.text });
    ...
    try {
      ...
      if (msg.conversationId) {
        conversation = await Conversations.findById(msg.conversationId)
        ...
        // No conversationId provided by client — try to find or create a DM conversation
        if (!msg.recipient) {
          throw new Error('Either conversationId or recipient must be provided');
        }

        // Use findOneAndUpdate with upsert to ensure conversation exists
        const recipientsArr = [msg.sender, msg.recipient];
        conversation = await Conversations.findOneAndUpdate(
          {
            $or: [
              { recipients: recipientsArr },
              { recipients: recipientsArr.slice().reverse() }
            ]
          },
          {
            recipients: recipientsArr,
        ...
        ).populate('recipients', 'fullname username avatar').populate('group');
        console.log('🔁 Conversation upsert result:', {
          conversationId: conversation?._id,
          recipients: conversation?.recipients?.map(r => r._id || r)
        });
      }
      ...
      if (conversation.isGroupConversation) {
        // Broadcast to all group members using the group message event name the client expects
        socket.to(`group_${conversation.group._id}`).emit('addGroupMessageToClient', savedMessage);
        // Do NOT emit to the sender socket here — the sender receives the saved message via the callback acknowledgement
      } else {
        // Broadcast to DM recipient using the event name the client expects
          const recipientId = conversation.recipients.find(
            r => r._id.toString() !== msg.sender.toString()
          )?._id.toString();

          console.log('Current users mapping:', users.map(u => ({ id: u.id, socketId: u.socketId })));
          const user = users.find(user => user.id === recipientId);
          if (user) {
            console.log(`✅ Recipient ${recipientId} is online, broadcasting message to socketId: ${user.socketId}`);
            socket.to(`${user.socketId}`).emit("addMessageToClient", savedMessage);
        ...
        // Do NOT emit to the sender here — the sender receives the saved message via the callback acknowledgement
      }
  ...
  socket.on('stopTyping', data => {
    ...
    if (user) {
      ...
      // send the 'from' field from the incoming data to the recipient
      socket.to(`${user.socketId}`).emit('stopTypingToClient', { from: data.from });
    }
  });
